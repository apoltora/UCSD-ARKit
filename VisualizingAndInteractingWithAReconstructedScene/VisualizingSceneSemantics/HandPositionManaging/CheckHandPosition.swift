...
public class CheckHandPosition {
    ...
    private var joints:[HandTracker2D.HandJointName : CGPoint]
    
    public init(joints: [HandTracker2D.HandJointName : CGPoint]) {
    ...
    private func returnCurrentPosition() -> HandPosition? {
        for position in self.possiblePositions {
    ...
    public func execute() {
        update()
        var currentPosition = returnCurrentPosition()
