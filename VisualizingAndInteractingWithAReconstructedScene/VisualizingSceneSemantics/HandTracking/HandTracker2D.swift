    ...
    fileprivate func updateTrackedViews(frame: ARFrame){
        ...
        for view in trackedViews {
            ...
            if let screenPosition = jointScreenPositions[jointIndex] {
                ...
                case .half, .quarter:
                    let difference = CGPoint(x:screenPosition.x - viewCenter.x,y:screenPosition.y - viewCenter.y)
                    view.value.center = CGPoint(x:viewCenter.x + (difference.x  * 0.5), y: viewCenter.y + (difference.y  * 0.5))
                }
            }
        }
    }
...
class SampleBufferDelegate {
    ...
    var handTrackers = [HandTracker2D]() {
        didSet {
