...
//    }
    ...
    private func checkPosition() {
        // want to check if the forefinger and thumb are in a pinched position
        // we will later make this into a separate class that
        guard var positions = twoDHandTracker.jointScreenPositions else { return }
        
        var handPosition = CheckHandPosition(joints: positions)

        // do different things based on the handPosition, but should all execute
        handPosition.execute()
